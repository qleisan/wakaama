...
def parse_client_registration(server_output):
    ...
    action, = re.findall(r"(registered|updated)", server_output)
    ...
    return int(client_id), action, endpoint, version, binding, int(lifetime), objects.string
...
def test_registration_interface(lwm2mserver, lwm2mclient):
    ...
    client_id, action, endpoint, version, binding, lifetime, objects = \
...
def test_registration_update_trigger(lwm2mserver, lwm2mclient):
    ...
    #lwm2mserver.expect_exact("updated")
    #print(lwm2mserver.before)
    client_id, action, endpoint, version, binding, lifetime, objects = \
        parse_client_registration(lwm2mserver.before)
    assert client_id == 0
