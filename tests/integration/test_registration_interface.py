...
def parse_client_registration(server_output):
    ...
    """
    client_id,  = re.findall(r"Client #([0-9]+) updated.", server_output)
    ...
    return int(client_id), endpoint, version, binding, int(lifetime), objects.string


def tXXest_registration_interface(lwm2mserver, lwm2mclient):
